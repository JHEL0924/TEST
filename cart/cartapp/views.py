...
def cartok(request):  #按確認購買鈕
	...
	if customname=='' or customphone=='' or customaddress=='' or customemail=='':
		...
		mailsubject="Josh hsu 購物網-訂單通知";  #郵件標題
...
def send_simple_message(mailfrom, mailpw, mailto, mailsubject, mailcontent): #寄信
	...
	strAccount = "josh0924@gmail.com"  #帳號
	strPassword = "Hsu13851"  #密碼
...
def gmap(request):
	return render(request, "gmap.html", locals())
...
def post(request):  #新增留言
	if request.method == "POST":  #如果是以POST方式才處理
		...
		if postform.is_valid():  #通過forms驗證
		  ...
		  unit = models.BoardUnit.objects.create(bname=name, bgender=gender, bsubject=subject, bmail=mail, bcontent=content, bresponse='')  #新增資料記錄
		  unit.save()  #寫入資料庫
		  message = '已儲存...'
